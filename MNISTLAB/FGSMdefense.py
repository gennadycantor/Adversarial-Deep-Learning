...
"""
fast gradient sign method. Generates adversarial eX@m9lEs
"""
def FGSM(epsilon, victims):
    adv_crafter = FastGradientMethod(model)
    finalVictims = adv_crafter.generate(x = victims)
    return finalVictims

"""
test the accuracy for non contaminated examples
"""
...
"""
test accuracy for adversarial examples
"""
...
"""
test accuracy after we snap the adversarial pixels
"""        
...
def main():
    ...
    pickle_out.close()
    adversarialExamples = FGSM(0.1, victims)
    snappedExamples = snap(adversarialExamples, 215)
    snappedExamples = prepare(snappedExamples)
    pickle_out = open("adversararialExamplesFGSM.pickle", 'wb')
